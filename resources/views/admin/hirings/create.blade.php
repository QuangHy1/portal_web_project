    ...
    <div class="container">
        ...
        <form action="{{ route('admin.hirings.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" id="description" name="description" rows="5" placeholder="Nhập mô tả" required>{{ old('description') }}</textarea>
                @error('description')
                <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="salary_range_id" name="salary_range_id">
                    ...
                    @foreach ($salaryRanges as $salaryRange)
                        <option value="{{ $salaryRange->id }}" {{ old('salary_range_id') == $salaryRange->id ? 'selected' : '' }}>
                            {{ $salaryRange->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                <label for="job_type_id" class="form-label">Hình thức làm việc</label>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="experience_id" name="experience_id" required>
                    ...
                    @foreach ($experiences as $experience)
                        <option value="{{ $experience->id }}" {{ old('experience_id') == $experience->id ? 'selected' : '' }}>
                            {{ $experience->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="tags" name="tags" value="{{ old('tags') }}" placeholder="Nhập các tags (cách nhau bởi dấu phẩy)">
                @error('tags')
                <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="education" name="education" value="{{ old('education') }}" placeholder="Nhập trình độ học vấn" required>
                @error('education')
                <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="isBoosted" name="isBoosted" required>
                    <option value="">Chọn</option>
                    <option value="yes" {{ old('isBoosted') == 'yes' ? 'selected' : '' }}>Có</option>
                    <option value="no" {{ old('isBoosted') == 'no' ? 'selected' : '' }}>Không</option>
                </select>
                @error('isBoosted')
                <div class="text-danger">{{ $message }}</div>
                @enderror
